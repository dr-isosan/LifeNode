...
def main():
    ...
    network.create_network(num_nodes=NUM_NODES, 
    ...
    try:
        ...
        visualizer.plot_network(network, 
                               title="LifeNode Ağ Topolojisi - İlk Hafta",
                               show_labels=True)
        
        # Başarılı paket yollarını göster
        if network.delivered_packets:
            print(f"   2. Teslim edilen paket yolları çiziliyor ({len(network.delivered_packets)} adet)...")
