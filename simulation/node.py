...
class Node:
    ...
    def __init__(self, node_id: int, position: Tuple[float, float]):
        ...
        # AI için ek özellikler
        self.signal_quality = 0.9  # Sinyal kalitesi (0.0-1.0)
        self.queue_capacity = 20  # Maksimum buffer kapasitesi

    @property
    def battery_level(self) -> float:
        """Enerji seviyesini 0-1 arası normalize et (AI için)"""
        return self.energy / 100.0

    @property
    def queue_len(self) -> int:
        """Buffer'daki paket sayısı (AI için)"""
        return len(self.buffer)

    def distance_to(self, other_node) -> float:
        """
        Başka bir düğüme Euclidean mesafe hesapla

        Args:
            other_node: Hedef Node objesi veya (x, y) tuple
        ...
        if isinstance(other_node, Node):
            ...
            other_pos = other_node  # (x, y) tuple
    ...
    def __str__(self):
        ...
        return (
            f"Node {self.id}: {self.position} - {status} - {len(self.neighbors)} komşu"
        )
